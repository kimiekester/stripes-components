...
class MCLRenderer extends React.Component {
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    if (receivedRows !== contentData.length) {
      ...
      console.log(`MCL -- more data received ${receivedRows} to ${contentData.length}`);
    ...
    if (this.props.virtualize) {
      if (this.scrollTimeout) {
        ...
        this.setState({ scrolling: true }, () => {
          this.scrollTimeout = setTimeout(this.clearScrollingStatus(), 200);
        });
      }
  ...
  _loadMore(askAmount, index) {
    ...
    if (!this.state.loading &&
      ...
      onNeedMoreData &&
      !this.asked.includes(index)) {
      onNeedMoreData(askAmount, index);
      this.setState({ loading: true });
      this.asked.push(index);
      console.log(`MCL -- index ${index} asking`);
    }
  }
